 ...
 * mediante el algoritmo de corte mÃnimo de Stoer y Wagner
 ************************************************************************************/
...
#include <ilcplex/ilocplex.h> // LibrerÃa de CPLEX
...
public:
	...
	void Nesting(vector<int> &S1, vector<int> &S4, vector<vector<int>> &SS, int q) const
	{
		for (int j = 0; j < instance.Cluster[S4[q]].getSize(); j++)
		{
			//ajusto valor del vector para poder escribir
			S1[q] = instance.Cluster[S4[q]][j];

			if (q > 0)//condicion de anidacion
			{
				Nesting(S1, S4, SS, q - 1);
			}

			if (q == 0)//condicion de parada
			{
				SS.push_back(S1);
			}
		}
		...
		SeparateAlgorithmCuts(const IloCplex::Callback::Context &context) const
	{
		...
		for (int i = 0; i < instance.cardN; ++i)
		{
			...
			{
				X_Sol[i][j] = context.getCandidatePoint(X[i][j]);
			}
		}
		...
		for (int i = 0; i < instance.cardN; ++i)
		{
			...
			for (int j = 0; j < instance.cardN; ++j)
			{
				y_Sol[i][j] = context.getCandidatePoint(y[i][j]);
			}
		}
		...
		while (S2.size() != 0)
		{
			...
			for (int i = 0; i < S1.size(); i++)
			{
				for (int j = 0; j < S2.size(); j++)
				{
					...
					if (S1[i] != S2[j])
					{
						if (X_Sol[S1[i]][S2[j]] > eps_rhs || X_Sol[S2[j]][S1[i]] > eps_rhs) //si encuentro un arco que conecte con nodo S1[i]
						{
							for (int p = 0; p < S1.size(); p++)//me aseguro que el nodo ya no estÈ en el ciclo
							{
								if (S1[p] == S2[j])
									l++;
							}
			...
			//cout << endl << "Identifico subtour" << endl;
			//Utilizo S3 como vector de traspaso, para actualizar S2
			S3.clear();
			...
			//Creo en S5 el complemento de los clusters para S4
			S5.clear();
			 ...
			 */
			 ...
			 /***************(altere valores para hacerlo funcionar)**************************/
			 /*
			  S4.clear();
			  S1.clear();
			  S4.push_back(0);
			  ...
			  */
			  ...
			  //Condicion de Entrada: Ciclo cerrado, que no sea de un elemento y que no sea del tamaÒo de la red (TSP)
			if (S1.size() >= 2 && S1.size() < instance.clusterN)
			{
				/*
				vector<vector<int>> SS;//vector que almacena los nodos donde introducire los cortes

				int q = 0;
				q = S4.size() - 2;

				//aqui genero la andacion para determinar todas las posibles combinaciones de rutas en el subtour identificado
				for (int p = 0; p < instance.Cluster[S4[S4.size() - 1]].getSize(); p++)
				{
					S1[S4.size() - 1] = instance.Cluster[S4[S4.size() - 1]][p];
					Nesting(S1, S4, SS, q);
				}
				*/
				//Muestro todas las combinaciones creadas
				/*
				cout << endl << "combinaciones de rutas: " << endl;
				...
				*/
				...
				// Cortes Packing (nuevo enfoque)//
				/*
				for (int p = 0; p < SS.size(); p++)
				{
					IloExpr Cut_P(X.getEnv());
					IloNum S = 0;
					for (int i = 0; i < SS[p].size(); i++)
					{
						S++;
						for (int j = 0; j < SS[p].size(); j++)
						{
							if (SS[p][i] != SS[p][j])
								Cut_P += X[SS[p][i]][SS[p][j]];
						}
					}
					//context.rejectCandidate(Cut_P <= S - 1);
					//cout << endl << "Corte: " << endl;
					//cout << endl << Cut_P << " <= " << S - 1 << endl;
					Cut_P.end();
					//Contador que me dice cuantos cortes introduzco
					SEC++;
				}

				SS.clear(); //limpio los nodos para o repetir cortes
				*/
				//Corte Packing (antiguo enfoque)//
...
bool Load_data(IloEnv &env, char data_filename[255], Data &instance) // carga datos
{
	...
	if (f_input)
	{
		...
		/*
		 ...
		 cout << endl << "Vector de clusters:" << endl;
		 for (int i = 0; i < instance.cardN; i++)
		 {
		 cout << endl << "Nodo: " << i+1 << " en cluster: " << instance.SS[i];
		 }
...
bool Create_MIP(Data &instance, IloModel &Model, IloArray<IloBoolVarArray> &X, IloArray<IloBoolVarArray> &y, IloArray<IloNumVarArray> &f) // crea la instancia del modelo
{
	...
	/**********************Restricciones del Modelo de Asignaci€n (Por cluster)**********************/
	...
	// Restriccion (5) el tour tiene tantos arcos como cluster hay. Con esta restriccion se impone que el tour visita un solo nodo de cada cluster
	IloExpr arcs(env);
	for (int i = 0; i < instance.cardN; ++i)
	{
		for (int j = 0; j < instance.cardN; ++j)
...
bool MIP_solve(IloModel &Model, IloCplex &Cplex, Data &instance, IloArray<IloBoolVarArray> &X, IloArray<IloBoolVarArray> &y, IloArray<IloNumVarArray> &f)
{
	...
	Cplex.setParam(IloCplex::ClockType, 2); // 1: cpu time // 2: wall clock time // 0: defaul (wall clock time)
...
void Guardar(char *nombre, char resultados[50], IloCplex &Cplex, double &time, IloArray<IloBoolVarArray> &X, IloArray<IloBoolVarArray> &y)
{
	...
	if (salida2.is_open())
	{
		salida2 << instancia << "\t" << Cplex.getObjValue() << "\t" << Cplex.getMIPRelativeGap() << "\t" << Cplex.getNnodes() << "\t" << time << "\t" << SEC << endl;
	}
	else {
