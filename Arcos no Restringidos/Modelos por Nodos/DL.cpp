...
bool Create_MIP(Data &instance, IloModel &Model, IloArray<IloBoolVarArray> &X, IloArray<IloBoolVarArray> &y, IloIntVarArray &t) // crea la instancia del modelo
{
	...
	//t_i
	t = IloIntVarArray(env, instance.cardN, 1, instance.cardN - 1); //Definir como double ????
	...
	//Res (13): definicion de variable cola izquierda
	for (int i = 1; i < instance.cardN; i++)
	{
		IloExpr Asig_sum(env);
		IloExpr In_deeg(env);
		...
		Model.add(1 + (Asig_sum - 3) * X[i][0] + In_deeg <= t[i]);
		Asig_sum.end();
		In_deeg.end();
	}

	//Res (14): definicion de variable cola derecha
	for (int i = 1; i < instance.cardN; i++)
	{
		IloExpr Asig_sum(env);
		IloExpr Out_deeg(env);
		...
		Model.add(t[i] <= Asig_sum - 1 - (Asig_sum - 3) * X[0][i] - Out_deeg);
		Asig_sum.end();
		Out_deeg.end();
	}

	//Res (15): Eliminacion de subtour
	for (int i = 0; i < instance.cardN; i++)
	{
		for (int j = 1; j < instance.cardN; j++)
		{
			if (j != i)
			{
				Model.add(t[j] >= t[i] + X[i][j] - X[j][i] + (instance.cardN - 1) * (X[i][j] + X[j][i] - 1));
			}
