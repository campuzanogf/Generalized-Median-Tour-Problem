...
bool Create_MIP(Data &instance, IloModel &Model, IloArray<IloBoolVarArray> &X, IloArray<IloBoolVarArray> &y, IloArray<IloNumVarArray> &f) // crea la instancia del modelo
{
	...
	//f_ij
	f = IloArray<IloNumVarArray>(env, instance.cardN);

	for (int i = 0; i < instance.cardN; ++i) // cada fila
	{
		f[i] = IloNumVarArray(env, instance.cardN, 0, instance.cardN); // es un arreglo de N variables clusterdN
		for (int j = 0; j < instance.cardN; ++j)
		{
	...
	/**************Restriciones de Flujo (por nodos clásico)***************/

	// Restriccion (9): Flujo desde el origen
	IloExpr Out_depot(env);
	IloExpr Asig_dep(env);
	for (int j = 0; j < instance.cardN; ++j)
	{
		if (j != 0)
		{
	...
	Model.add(Out_depot == Asig_dep - 1); // sum{p} f_0p = 1
	Out_depot.end();
	Asig_dep.end();

	// Restriccion (10): balance de flujo
	for (int j = 1; j < instance.cardN; ++j)
	{
		...
		Model.add(In_cluster == y[j][j] + Out_cluster);
		In_cluster.end();
		Out_cluster.end();
	}

	// Restriccion (11): Flujo en arcos
	for (int i = 0; i < instance.cardN; ++i)
	{
		for (int j = 0; j < instance.cardN; ++j)
		{
			if (j != i)
			{
				Model.add(f[i][j] <= (instance.cardN - 1) * X[i][j]);
			}
		}
	}
